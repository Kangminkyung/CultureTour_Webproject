...
<div id="mySidenav" class="sidenav"> <a href="javascript:void(0)" class="closebtn" onclick="closeNav()">&times;</a>
  <nav>
    <ul>
      ...
      <li><a href="<%=request.getContextPath()%>/memberDetail.action?idx=${sessionScope.loginuser.idx}">나의정보</a>
        <ul class="sub-menu">
          <li><a href="<%=request.getContextPath()%>/memberEdit.action?idx=${sessionScope.loginuser.idx}" >개인정보수정</a></li>
          <li><a href="<%=request.getContextPath()%>/CultureWishList.action" >나의 관심문화재</a></li>   
          <li><a href="<%=request.getContextPath()%>/mybandList.action" >나의 밴드보기</a></li>   
        </ul>
	  </li>
      </c:if>
      
      <c:if test="${sessionScope.loginuser != null && (sessionScope.loginuser).userid eq 'admin'}">
     		 <li><a href="<%=request.getContextPath()%>/memberlist.action">회원정보보기</a></li>
      </c:if>
      
      <li><a>커뮤니티</a>
        <ul class="sub-menu">
         <li><a href="<%=request.getContextPath()%>/bandList.action" >밴드</a></li> 
         <li><a href="<%=request.getContextPath()%>/quiz.action" >퀴즈</a></li>
         <li><a href="<%=request.getContextPath()%>/wordCloud.action" >검색어차트</a>
        </ul>
      </li>
      <li><a>게시판</a>
        <ul class="sub-menu">
         <li><a href="<%=request.getContextPath()%>/showNotice.action" >공지사항</a></li> 
         <li><a href="<%=request.getContextPath()%>/qnalist.action" >자유게시판</a></li>
        </ul>
      </li>
  ...
  &nbsp;&nbsp;<input type="text" id="cultureSearch" name="cultureSearch" value="${cultureSearchWord}" style="width: 185px;"></input>
	<button type="button" style="width: 3px; text-align: center; background-color: #468EDE; color: #FFFFFF;" onclick="goSearch2();"><i class="fa fa-search"></i></button>
